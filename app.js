...
function openEdit(id){const u=users.find(x=>x.id===id);if(!u)return;editForm.id.value=u.id;editForm.name.value=u.name;editForm.email.value=u.email;editForm.role.value=u.role;editForm.status.value=u.status;editForm.emailVerified.checked=Boolean(u.emailVerified);editBackdrop.querySelector('#adminEditTitle').textContent=`Редактировать: ${u.name}`;editBackdrop.classList.add('open');}
...
editForm.addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(editForm);const id=Number(f.get('id'));try{await api(`/api/users/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:f.get('name'),email:f.get('email'),role:f.get('role'),status:f.get('status'),emailVerified:editForm.emailVerified.checked})});editBackdrop.classList.remove('open');showToast('Данные пользователя обновлены');await loadUsers();}catch(err){showToast(err.message,'error');}});
