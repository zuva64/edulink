...
async function checkSession(){const r=await fetch('/api/session');if(!r.ok)return;const a=await r.json();if(a.role!=='admin'){redirectByRole(a.role);return;}document.querySelector('#authScreen').classList.add('hidden');document.dispatchEvent(new Event('edulink:authenticated'));await loadUsers();}

document.querySelector('#loginForm').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.target);try{const a=await api('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:f.get('email'),password:f.get('password')})});if(a.role!=='admin'){redirectByRole(a.role);return;}document.querySelector('#authScreen').classList.add('hidden');document.dispatchEvent(new Event('edulink:authenticated'));await loadUsers();}catch(err){document.querySelector('#authError').textContent=err.message;}});
